...
object User {
  case class Client(version: Option[String],
                    `type`: Option[String])
  object Client { implicit val reads = Json.reads[Client] }

  case class Presence(status: Option[String],
                      idle: Option[Int],
                      show: Option[Show],
...
case class User(xmpp_jid: String,
                is_deleted: Boolean,
                name: String,
...
case class UserBrief(mention_name: String,
                     id: Int,
                     links: SelfLinks,
                     name: String)
